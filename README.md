...
- `llm-client` — `LLMClient` interface with `GeminiClient` (default provider, `gemini-3.6-flash`) and `OllamaClient` (optional, for local/offline use) implementations, selected by `LLMClientFactory` -- the *only* place in the codebase that knows both exist. No agent, prompt, or service ever branches on which provider is active.
